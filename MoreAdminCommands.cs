...
namespace MoreAdminCommands
{
    ...
    public class MoreAdminCommands : TerrariaPlugin
    {
        ...
        public static void FindCommand(CommandArgs args)
        {
            ...
            if (args.Parameters.Count > 0)
            {
                ...
                foreach (Command command in Commands.ChatCommands)
                {
                    ...
                    if (args.Player.Group.HasPermission("findperm"))
                    {
        ...
        private void OnLeave(LeaveEventArgs e)
        {
            ...
            flyMode[e.Who] = false;
            muted[e.Who] = false;
            muteAllFree[e.Who] = false;
            viewAll[e.Who] = false;
            foreach (Vector2 entry in carpetPoints[e.Who])
            {

                Main.tile[(int)entry.X, (int)entry.Y].active(false);

            }
            carpetPoints[e.Who] = new List<Vector2>();
            buffsUsed[e.Who] = new List<int>();
            accessRed[e.Who] = (redPass == "");
            accessBlue[e.Who] = (bluePass == "");
            accessGreen[e.Who] = (greenPass == "");
            accessYellow[e.Who] = (yellowPass == "");
            autoKill[e.Who] = false;
            tpOff[e.Who] = false;
        }
        ...
        public void OnChat(ServerChatEventArgs e)
        {
            ...
            if (findIfPlayingCommand(text) && !TShock.Players[e.Who].Group.HasPermission("ghostmode"))
            {
                ...
                TShock.Players[e.Who].SendMessage(string.Format("Current players: {0}.", sb), 255, 240, 20);
                e.Handled = true;

            }
            if (((muted[e.Who]) && (findIfMeCommand(text))) || ((muteAll) && (!TShock.Players[e.Who].Group.HasPermission("mute"))))
            {

                TShock.Players[e.Who].SendMessage("You cannot use the /me command, you are muted.", Color.Red);
                e.Handled = true;
                return;
            ...
            if ((muted[e.Who] || muteAll) && !TShock.Players[e.Who].Group.HasPermission("mute"))
            {
